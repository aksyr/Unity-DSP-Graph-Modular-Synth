...
namespace XNode {
    ...
    public class NodePort {
        ...
        public bool CanConnectTo(NodePort port) {
            ...
            // check custom node constraints
            if (!node.CanConnect(output, input)) { return false; }
            if (!port.node.CanConnect(output, input)) { return false; }
            // Success
            return true;
        }
        ...
        public void Disconnect(NodePort port) {
            ...
            NodePort input = IsOutput ? port : this;
            NodePort output = IsOutput ? this : port;
            node.OnRemoveConnection(output, input);
            if (port != null) port.node.OnRemoveConnection(output, input);
        }
        ...
        public void Disconnect(int i) {
            ...
            // Trigger OnRemoveConnection
            NodePort input = IsOutput ? otherPort : this;
            NodePort output = IsOutput ? this : otherPort;
            node.OnRemoveConnection(output, input);
            if (otherPort != null) otherPort.node.OnRemoveConnection(output, input);
        }
