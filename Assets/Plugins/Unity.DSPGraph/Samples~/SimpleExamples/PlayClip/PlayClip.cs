    ...
    struct PlayClipNode : IAudioKernel<PlayClipNode.Parameters, PlayClipNode.SampleProviders>
    {
        ...
        public void Execute(ref ExecuteContext<Parameters, SampleProviders> context)
        {
            if (Playing)
            {
                ...
                // true.
                var finished = Resampler.ResampleLerpRead(provider, ResampleBuffer, buffer, context.Parameters,
                    Parameters.Rate);
        ...
        void OnGUI()
        {
            if (GUI.Button(new Rect(10, 10, 150, 100), "Play Clip!"))
            {
                ...
                using (var block = m_Graph.CreateCommandBlock())
                {
                    ...
                    block.UpdateAudioKernel<PlayClipKernel, PlayClipNode.Parameters, PlayClipNode.SampleProviders,
                                            PlayClipNode>(new PlayClipKernel(), m_Node);
                }
            }
        }
    }
