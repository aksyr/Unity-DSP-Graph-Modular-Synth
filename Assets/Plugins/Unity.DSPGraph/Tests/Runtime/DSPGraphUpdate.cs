            ...
            using (var graph = DSPGraph.Create(soundFormat, channelCount, 100, 1000))
            {
                ...
                using (var buff = new NativeArray<float>(200, Allocator.Temp))
                {
                    ...
                    using (var block = graph.CreateCommandBlock())
                    {
                        ...
                        block.UpdateAudioKernel<TestValueKernelUpdater, NoParameters, NoProviders, TestUpdateKernel>(
                            kernelUpdater, node);
                    }
        ...
        public void UpdateRequestCallsDelegate(DSPGraph.ExecutionMode executionMode)
        {
            ...
            using (var graph = DSPGraph.Create(soundFormat, channelCount, 100, 1000))
            {
                ...
                using (var buff = new NativeArray<float>(200, Allocator.Temp))
                {
                    ...
                    var called = false;
                    DSPNodeUpdateRequest<NullAudioKernelUpdater, NoParameters, NoProviders, NullAudioKernel>
                    updateRequest;
                    using (var block = graph.CreateCommandBlock())
                        updateRequest =
                            block.CreateUpdateRequest<NullAudioKernelUpdater, NoParameters, NoProviders, NullAudioKernel
                                >(
