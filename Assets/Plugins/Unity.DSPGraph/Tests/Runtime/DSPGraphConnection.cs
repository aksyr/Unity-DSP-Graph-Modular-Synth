...
namespace Unity.Audio.DSPGraphTests
{
    public class DSPGraphConnection
    {
        ...
        struct TestData : IDisposable
        {
            ...
            public static TestData Create(bool createGenerator = true, int channelCount = DefaultChannelCount,
                SoundFormat soundFormat = DefaultSoundFormat,
                DSPGraph.ExecutionMode executionMode = DSPGraph.ExecutionMode.Jobified)
            {
                var graph = DSPGraph.Create(soundFormat, channelCount, DefaultBufferSize, 1000);
                NodeData nodeData;
                using (var block = graph.CreateCommandBlock())
                    nodeData = createGenerator
                        ? CreateGeneratorNode(block, graph.RootDSP, channelCount, soundFormat)
                        : CreatePassthroughNode(block, graph, channelCount, soundFormat);
                return new TestData
                {
                    Graph = graph,
                    Node = nodeData.Node,
            ...
            public static NodeData CreateGeneratorNode(DSPCommandBlock block, DSPNode consumer, int channelCount,
            ...
            public static NodeData CreatePassthroughNode(DSPCommandBlock block, DSPGraph graph, int channelCount,
                SoundFormat soundFormat)
            {
                var node = block.CreateDSPNode<NoParameters, NoProviders, PassThrough>();
                block.AddInletPort(node, channelCount);
                block.AddOutletPort(node, channelCount);
            ...
            public NodeData CreateGeneratorNode(DSPNode consumer, int channelCount = DefaultChannelCount,
                SoundFormat soundFormat = DefaultSoundFormat)
            {
                using (var block = Graph.CreateCommandBlock())
            ...
            public NodeData CreateGeneratorNode(int channelCount = DefaultChannelCount,
                SoundFormat soundFormat = DefaultSoundFormat)
            {
                ...
                using (var buff = new NativeArray<float>(DefaultBufferSize * channelCount, Allocator.Temp))
                {
                    ...
                    for (int i = 0; i < sampleFrameCount; ++i)
                    {
                        for (int c = 0; c < channelCount; ++c)
                            Assert.That(buff[c * sampleFrameCount + i], Is.EqualTo(expectedValue[c]));
                    }
                }
            }
        ...
        {
            using (var testData = TestData.Create(true, TestData.DefaultChannelCount, TestData.DefaultSoundFormat,
                executionMode))
        ...
        [TestCase(DSPGraph.ExecutionMode.Jobified)]
        [TestCase(DSPGraph.ExecutionMode.Synchronous)]
        public void AttenuatedInput_ProducesMultiChannelAttenuatedResults_InInputPorts(
        ...
        [TestCase(DSPGraph.ExecutionMode.Jobified)]
        [TestCase(DSPGraph.ExecutionMode.Synchronous)]
        public void AttenuatedInterpolatedInput_ProducesMultiChannelInterpolatedResults_InInputPorts(
            DSPGraph.ExecutionMode executionMode)
        {
            var channelCount = 2;
            var soundFormat = SoundFormat.Stereo;
        ...
        public void MultiNoopInput_ProducesSummedAttenuatedResults_InInputPorts(DSPGraph.ExecutionMode executionMode)
        {
            ...
            using (var testData = TestData.Create(false, TestData.DefaultChannelCount, TestData.DefaultSoundFormat,
                executionMode))
            {
        ...
        public void InterruptedAttenuation_ContinuesWithAttenuatedValue(DSPGraph.ExecutionMode executionMode)
        {
            ...
            using (var testData = TestData.Create(true, channelCount, soundFormat, executionMode))
            {
                ...
                using (var buff = new NativeArray<float>(TestData.DefaultBufferSize * channelCount, Allocator.Temp))
                {
                    ...
                    float[] attenuationDelta =
                    {
                        (-1 / ((float)(TestData.DefaultBufferSize * 2) - 1)),
                        (-1 / ((float)(TestData.DefaultBufferSize * 2) - 1))
                    };

                    testData.Mix(buff, buff.Length / channelCount, channelCount);
                    VerifyInterpolatedAttenuation(buff, 0, TestData.DefaultBufferSize, channelCount,
                        baseAttenuationValue, attenuationDelta);
                    ...
                    Assert.AreEqual(buff[(TestData.DefaultBufferSize * channelCount) - 1], baseAttenuationValue[0],
                        0.0001);

                    testData.Attenuate(testData.Connection, DSPConnection.DefaultAttenuation,
                        TestData.DefaultBufferSize);
                    testData.Mix(buff, buff.Length / channelCount, channelCount);
        ...
        public void UnattachedSubtreesAreExecutedWithModeEnabled(DSPGraph.ExecutionMode executionMode)
        {
                ...
                executionMode))
            {
                ...
                using (var block = testData.Graph.CreateCommandBlock())
                {
                    ...
                    block.AddOutletPort(testData.Node, TestData.DefaultChannelCount);
                    block.AddInletPort(unattachedNode, TestData.DefaultChannelCount);
                    block.AddOutletPort(unattachedNode, TestData.DefaultChannelCount);
        ...
        public void ConnectDisconnectClearsState()
        {
            using (var testData = TestData.Create(false))
            {
                ...
                using (var block = testData.Graph.CreateCommandBlock())
                {
                    ...
                    block.AddOutletPort(source, TestData.DefaultChannelCount);

                    destination = block.CreateDSPNode<NoParameters, NoProviders, NullAudioKernel>();
                    block.AddInletPort(destination, TestData.DefaultChannelCount);
        ...
        public void IncompatiblePortsThrowsException()
        {
            using (var testData = TestData.Create(false))
            {
                ...
                {
                    source = block.CreateDSPNode<NoParameters, NoProviders, NullAudioKernel>();
                    block.AddOutletPort(source, 1);

                    destination = block.CreateDSPNode<NoParameters, NoProviders, NullAudioKernel>();
                    block.AddInletPort(destination, 4);
        ...
        public void CreatingDirectCycleThrowsException()
        {
            using (var testData = TestData.Create(false))
            {
                ...
                using (var block = testData.Graph.CreateCommandBlock())
                {
                    source = block.CreateDSPNode<NoParameters, NoProviders, NullAudioKernel>();
                    block.AddOutletPort(source, TestData.DefaultChannelCount);
                    block.AddInletPort(source, TestData.DefaultChannelCount);

                    destination = block.CreateDSPNode<NoParameters, NoProviders, NullAudioKernel>();
                    block.AddOutletPort(destination, TestData.DefaultChannelCount);
                    block.AddInletPort(destination, TestData.DefaultChannelCount);
        ...
        public void CreatingIndirectCycleThrowsException()
        {
            using (var testData = TestData.Create(false))
            {
                ...
                using (var block = testData.Graph.CreateCommandBlock())
                {
                    ...
                    block.AddOutletPort(a, TestData.DefaultChannelCount);
                    block.AddInletPort(a, TestData.DefaultChannelCount);

                    b = block.CreateDSPNode<NoParameters, NoProviders, NullAudioKernel>();
                    block.AddOutletPort(b, TestData.DefaultChannelCount);
                    block.AddInletPort(b, TestData.DefaultChannelCount);

                    c = block.CreateDSPNode<NoParameters, NoProviders, NullAudioKernel>();
                    block.AddOutletPort(c, TestData.DefaultChannelCount);
                    block.AddInletPort(c, TestData.DefaultChannelCount);

                    d = block.CreateDSPNode<NoParameters, NoProviders, NullAudioKernel>();
                    block.AddOutletPort(d, TestData.DefaultChannelCount);
                    block.AddInletPort(d, TestData.DefaultChannelCount);
        ...
        public void FindConnection_WithValidNonzeroPort_Succeeds()
        {
            using (var testData = TestData.Create(false))
            {
                ...
                testData.CheckMix(0.0f);
                Assert.AreEqual(DSPConnection.InvalidIndex,
                    testData.Graph.FindConnectionIndex(c.Handle.Id, 0, a.Handle.Id, 1));
                Assert.AreEqual(DSPConnection.InvalidIndex,
                    testData.Graph.FindConnectionIndex(c.Handle.Id, 1, a.Handle.Id, 0));
        ...
        public void Disconnect_InConnectionOrder()
        {
            using (var testData = TestData.Create(false))
            {
                ...
                using (var block = testData.Graph.CreateCommandBlock())
                {
                    // These will get autodisconnected when we release the node,
                    // but we also want to cover the manual disconnect codepath
                    block.Disconnect(a, 0, b, 0);
                    block.Disconnect(a, 0, c, 0);
                    block.Disconnect(a, 0, d, 0);
        ...
        static void VerifyInterpolatedAttenuation(NativeArray<float> buffer, int firstSample, int sampleCount,
            int channelCount, float[] baseAttenuationValue, float[] attenuationDelta)
        {
            ...
            for (int sample = firstSample; sample < sampleCount; ++sample)
            {
                ...
                {
                    var bufferIndex = (channel * samplesPerChannel) + sample;
                    Assert.AreEqual(baseAttenuationValue[channel] + (sample * attenuationDelta[channel]),
                        buffer[bufferIndex], 0.0001, $"Mismatched expectations at index {bufferIndex}");
                }
            }
        }
    }
