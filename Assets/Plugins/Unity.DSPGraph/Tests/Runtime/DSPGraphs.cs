...
namespace Unity.Audio.DSPGraphTests
{
    public class DSPGraphs
    {
        ...
        [TestCase(-1, 2, 200, 48000, false)]
        // Mismatched format/channelCount
        [TestCase(SoundFormat.Raw, 1, 200, 48000, false)]
        [TestCase(SoundFormat.Mono, 2, 200, 48000, false)]
        [TestCase(SoundFormat.Stereo, 1, 200, 48000, false)]
        [TestCase(SoundFormat.Quad, 2, 200, 48000, false)]
        [TestCase(SoundFormat.Surround, 2, 200, 48000, false)]
        [TestCase(SoundFormat.FiveDot1, 2, 200, 48000, false)]
        [TestCase(SoundFormat.SevenDot1, 2, 200, 48000, false)]
        // Invalid channel count
        [TestCase(SoundFormat.Raw, 0, 200, 48000, false)]
        // Invalid buffer size
        [TestCase(SoundFormat.Stereo, 2, 0, 48000, false)]
        // Invalid sample rate
        [TestCase(SoundFormat.Stereo, 2, 200, 0, false)]

        // Valid configs
        [TestCase(SoundFormat.Stereo, 2, 201, 48000, true)]
        [TestCase(SoundFormat.SevenDot1, 8, 100, 48000, true)]
        [TestCase(SoundFormat.Mono, 1, 200, 48000, true)]
        [TestCase(SoundFormat.Stereo, 2, 201, 48000, true)]
        [TestCase(SoundFormat.Quad, 4, 201, 48000, true)]
        [TestCase(SoundFormat.FiveDot1, 6, 200, 48000, true)]
        [TestCase(SoundFormat.SevenDot1, 8, 100, 48000, true)]
        public void DSPGraphCreateParameters_AreValidated(SoundFormat format, int channels, int bufferSize,
            int sampleRate, bool valid)
        {
            if (valid)
                Assert.DoesNotThrow(() => { DSPGraph.Create(format, channels, bufferSize, sampleRate); });
            else
                Assert.Throws<ArgumentOutOfRangeException>(() =>
                {
                    DSPGraph.Create(format, channels, bufferSize, sampleRate);
                });
        }
        ...
        public void MixOverwritesData(DSPGraph.ExecutionMode executionMode)
        {
            using (var setup = new GraphSetup((graphSetup, graph, block) =>
            {
                ...
                block.AddOutletPort(node, GraphSetup.ChannelCount);
                block.Connect(node, 0, graph.RootDSP, 0);
            }))
            {
                using (var buffer = new NativeArray<float>(setup.Graph.DSPBufferSize * GraphSetup.ChannelCount,
        ...
        public void MultipleBeginMix_Works(DSPGraph.ExecutionMode executionMode)
        {
            ...
            {
                using (var buffer =
                           new NativeArray<float>(setup.Graph.DSPBufferSize * GraphSetup.ChannelCount, Allocator.Temp))
                {
                    int i;
                    for (i = 0; i < 10; ++i)
        ...
        public void ReleasingDSPNode_InChain_KillsSignal(DSPGraph.ExecutionMode executionMode)
        {
            ...
            using (var setup = new GraphSetup((graphSetup, graph, block) =>
            {
                ...
                block.AddOutletPort(generator, GraphSetup.ChannelCount);

                passthrough = block.CreateDSPNode<NoParameters, NoProviders, PassThrough>();
                block.AddInletPort(passthrough, GraphSetup.ChannelCount);
                block.AddOutletPort(passthrough, GraphSetup.ChannelCount);
