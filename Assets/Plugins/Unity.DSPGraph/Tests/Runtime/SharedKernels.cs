...
namespace Unity.Audio.DSPGraphTests
{
    ...
    struct GenerateOne : IAudioKernel<NoParameters, NoProviders>
    {
        ...
        public void Execute(ref ExecuteContext<NoParameters, NoProviders> context)
        {
            ...
            {
                var sampleBuffer = context.Outputs.GetSampleBuffer(o);
                for (int channel = 0; channel < sampleBuffer.Channels; ++channel)
                {
                    var buffer = sampleBuffer.GetBuffer(channel);
    ...
    struct PassThrough : IAudioKernel<NoParameters, NoProviders>
    {
        ...
        public void Execute(ref ExecuteContext<NoParameters, NoProviders> context)
        {
            ...
            {
                var outBuf = output.GetBuffer(channel);
                var inBuf = input.GetBuffer(channel);
