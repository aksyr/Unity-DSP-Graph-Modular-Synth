...
namespace Unity.Audio.DSPGraphTests
{
    public abstract class AudioClipProvider : IPrebuildSetup
    {
        ...
        protected void ReadWholeClip_ReturnsAllSamples()
        {
            using (var provider = CreateAudioSampleProvider(m_AudioClip))
            {
                ...
                using (var buffer =
                           new NativeArray<float>(sampleFrameCountPerChunk * channelCount, Allocator.Persistent))
                {
            ...
            long endSampleFrameIndex = 0, bool loop = false, bool allowDrop = false)
        {
            return AudioSampleProvider.Lookup(
                audioClip.Internal_CreateAudioClipSampleProvider((ulong)startSampleFrameIndex, endSampleFrameIndex,
                    loop, allowDrop),
                null, 0);
        }
    }
    ...
    {
        [Test]
        public void ReadWholeClip_Returns_AllSamples()
        {
            ReadWholeClip_ReturnsAllSamples();
        }

        [Test]
        public void Looping_ReturnsSamples_Indefinitely()
        {
            Looping_ReturnsSamplesIndefinitely();
        }

        [Test]
        public void AllowedDropouts_ReturnsSamples_Indefinitely()
        {
            AllowedDropouts_ReturnsSamplesIndefinitely();
        }
