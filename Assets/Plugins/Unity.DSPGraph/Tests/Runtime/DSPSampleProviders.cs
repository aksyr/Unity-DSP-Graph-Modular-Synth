...
namespace Unity.Audio.DSPGraphTests
{
    public class DSPSampleProviders
    {
        ...
        {
            Single,
            [SampleProviderArray] VariableSizeArray,
            [SampleProviderArray(3)] FixedArray
        }
        ...
        void AddProviderToNode<TKernel>(DSPCommandBlock block, DSPNode node, int index = -1,
            float value = kSignalValueA, Providers item = Providers.VariableSizeArray, bool insert = true)
            where TKernel : struct, IAudioKernel<NoParameters, Providers>
        {
            ...
            var inputBuff = new NativeArray<float>(2 * kChannelCount * kDspBufferSize, Allocator.Temp,
        ...
        struct NoSampleProviderKernel : IAudioKernel<NoParameters, NoProviders>
        {
            ...
            public void Execute(ref ExecuteContext<NoParameters, NoProviders> context)
            {
                ...
                Assert.Throws<IndexOutOfRangeException>(() => providers.GetSampleProvider(0));
            }
        ...
        struct InitialSampleProviderKernel : IAudioKernel<NoParameters, Providers>
        {
            ...
            public void Execute(ref ExecuteContext<NoParameters, Providers> context)
            {
                ...
                Assert.Throws<IndexOutOfRangeException>(() => providers.GetSampleProvider(Providers.Single, 1));

                // Second item is a var array. These are born empty.
                Assert.Throws<IndexOutOfRangeException>(() =>
                    providers.GetSampleProvider(Providers.VariableSizeArray));
                ...
                Assert.Throws<IndexOutOfRangeException>(() => providers.GetSampleProvider(Providers.FixedArray, 3));
                Assert.Throws<IndexOutOfRangeException>(() => providers.GetSampleProvider(3));
            }
        ...
        struct InaccessibleLocalSampleProviderKernel : IAudioKernel<NoParameters, NoProviders>
        {
            ...
            public void Execute(ref ExecuteContext<NoParameters, NoProviders> context)
            {
                ...
                Assert.Throws<InvalidOperationException>(() => localProvider.Read(outputs.GetSampleBuffer(0)));
            }
        ...
        public void ProviderFields_MatchOriginalProvider()
        {
            using (CreateTestData<InitialSampleProviderKernel, Providers>(
                (block, node) =>
                {
                    AddProviderToNode<SingleSampleProviderKernel>(block, node, 0, kSignalValueA, Providers.Single,
                        false);
                }))
            {
            }
            ...
            using (CreateTestData<InitialSampleProviderKernel, Providers>(
                (block, node) =>
                {
                    Assert.Throws<InvalidOperationException>(() =>
                        AddProviderToNode<InitialSampleProviderKernel>(block, node, -1, kSignalValueA,
                            Providers.FixedArray, true));
                }))
            {
            }
        }
        ...
        public void FixedProviderArray_CannotBeShortened()
        {
            using (var testData = CreateTestData<InitialSampleProviderKernel, Providers>(
                ...
                {
                    Assert.Throws<InvalidOperationException>(() =>
                        block.RemoveSampleProvider<NoParameters, Providers, InitialSampleProviderKernel>(node,
                            Providers.FixedArray, 0));
                }))
            {
        ...
        public void FixedProviderArray_CanReceiveNewProvider()
        {
            using (CreateTestData<FixedArraySampleProviderKernel, Providers>(
                (block, node) =>
                {
                    AddProviderToNode<SingleSampleProviderKernel>(block, node, 0, kSignalValueA, Providers.FixedArray,
                        false);
                }))
            {
            }
        ...
        public void FixedProviderArray_CanClearProvider()
        {
                ...
                }))
            {
                using (var block = testData.Graph.CreateCommandBlock())
                    block.SetSampleProvider<NoParameters, Providers, InitialSampleProviderKernel>(0, testData.Node,
                        Providers.FixedArray, 0);
            }
        }
        ...
        struct OneSampleProviderKernel : IAudioKernel<NoParameters, Providers>
        {
            ...
            public void Execute(ref ExecuteContext<NoParameters, Providers> context)
            {
                ...
                var providers = context.Providers;
                Assert.Throws<IndexOutOfRangeException>(() =>
                    providers.GetSampleProvider(Providers.VariableSizeArray, 1));
                Assert.That(provider.Valid, Is.True);
        ...
        public void RemoveProvider_MakesVariableSampleProviderArrayEmpty()
        {
            ...
            {
                using (var block = testData.Graph.CreateCommandBlock())
                    block.RemoveSampleProvider<NoParameters, Providers, InitialSampleProviderKernel>(testData.Node,
                        Providers.VariableSizeArray, 0);
                CheckOutputBuffer(testData.Graph);
            }
        }
        ...
        struct ReadSampleProviderKernel : IAudioKernel<NoParameters, Providers>
        {
            ...
            public void Execute(ref ExecuteContext<NoParameters, Providers> context)
            {
                context.Providers.GetSampleProvider(Providers.VariableSizeArray, 0)
                    .Read(context.Outputs.GetSampleBuffer(0));
            }
        ...
        public void ReadSampleProvider_PlaysSignal(DSPGraph.ExecutionMode executionMode)
        {
            ...
            {
                CheckOutputBuffer(testData.Graph, (buffer) => CheckBufferHasValue(buffer, kSignalValueA),
                    executionMode);
            }
        }
        ...
        public void ReadOverwrittenSampleProvider_PlaysNewSignal(DSPGraph.ExecutionMode executionMode)
        {
                ...
                (block, node) => AddProviderToNode<ReadSampleProviderKernel>(block, node)))
            {
                ...
                CheckOutputBuffer(testData.Graph, (buffer) => CheckBufferHasValue(buffer, kSignalValueB),
                    executionMode);
            }
        }
        ...
        struct AddSampleProvidersKernel : IAudioKernel<NoParameters, Providers>
        {
            ...
            public void Execute(ref ExecuteContext<NoParameters, Providers> context)
            {
                ...
                var count = providers.GetCount(Providers.VariableSizeArray);
                for (int i = 0; i < count; ++i)
                    providers.GetSampleProvider(Providers.VariableSizeArray, i)
                        .Read(outputBuffer, writeMode: BufferWriteMode.Additive);
            }
        ...
        public void AddSampleProviders_PlaysAddedSignals(DSPGraph.ExecutionMode executionMode)
        {
                ...
                (block, node) => AddProviderToNode<AddSampleProvidersKernel>(block, node)))
            {
                ...
                CheckOutputBuffer(testData.Graph,
                    (buffer) => CheckBufferHasValue(buffer, kSignalValueA + kSignalValueB), executionMode);
            }
        }
        ...
        public void ClearProviderViaAudioClip()
        {
            using (var testData = CreateTestData<AddSampleProvidersKernel, Providers>())
            using (var block = testData.Graph.CreateCommandBlock())
                block.SetSampleProvider<NoParameters, Providers, AddSampleProvidersKernel>((AudioClip)null,
                    testData.Node, Providers.Single);
        }
        ...
        public void ClearProviderViaVideo()
        {
            using (var testData = CreateTestData<AddSampleProvidersKernel, Providers>())
            using (var block = testData.Graph.CreateCommandBlock())
                block.SetSampleProvider<NoParameters, Providers, AddSampleProvidersKernel>((VideoPlayer)null,
                    testData.Node, Providers.Single);
        }
    }
}
